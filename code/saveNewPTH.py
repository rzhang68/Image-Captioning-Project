...
model_name = 'model1_loadPre'  # model1:CNN+attension, LSTM+attension
...
def main():
    ...
    if checkpoint is None:
        model_path = 'test2.pth.tar'  # test.pth.tar
        ...
        # decoder_optimizer = decoder_optimizer.to(device)
        ...
        # encoder.load_state_dict(checkpoint['encoder']) ###error
        encoder_optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, encoder.parameters()),
                                             lr=encoder_lr) if fine_tune_encoder else None
        # encoder_optimizer = encoder_optimizer.to(device)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        save_checkpoint(data_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer,
                        decoder_optimizer, best_bleu4, is_best, checkpoint_folder, model_name)
